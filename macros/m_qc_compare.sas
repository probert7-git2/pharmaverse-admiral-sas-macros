/*--------------------------------------------------------------
  ...
  Program Name                : m_qc_compare.sas
  SAS Version                 : 9.4
  Purpose (short description) : Controlled merge compare of SAS ADaM vs R reference
  Author                      : Cursor Grok 4.5
  Date                        : 09AUG2026
  Input Datasets or Metadata  : adam.* and ref.ref_*
  Modification Log            : 23AUG2026 - Part B date-scale *DTM promotion moved
                                to SAS_mirrored_Admiral_safety_ADaM/SAS/macro/
                                m_qc_compare_pva.sas - Track A no longer promotes
                                *DTM (Part A gold already datetime-scale).
  ...
  *SEQ, so a single upstream difference reports as thousands of orphans.
  ...
  Instead every domain declares its own:
    ...
    sortkeys = business keys both builders can reproduce for THAT domain
               (BDS: USUBJID PARAMCD AVISIT ADT -
...
/* Part B date-scale *DTM promotion lives only in
   SAS_mirrored_Admiral_safety_ADaM/SAS/macro/m_qc_compare_pva.sas
   (%m_qc_norm_dtm_scale). Track A prep_pair does not call it. */
...
/*======================================================================
  ...
  builder sorted by USUBJID PARAMCD AVISIT ADT LBSEQ resolves to
  sortkeys = USUBJID PARAMCD AVISIT ADT and tiebreak = LBSEQ.
======================================================================*/
...
%macro m_qc_default_keys(label=, out_keys=, out_tie=);
  ...
  %else %if &l = ADVS %then %do;
    /* BDS finding only (not OCCDS). AVISIT = analysis visit per admiral
       bds_finding / ADaMIG - *SEQ tiebreak. */
    %let &out_keys = USUBJID PARAMCD AVISIT ADT;
    %let &out_tie  = VSSEQ;
  %end;
...
, label=
  ...
  data _qc_mis;
    length _dataset $16 _issue $40 _sas_val $40 _ref_val $40;
    ...
    keep _dataset _issue _sas_val _ref_val &_keys &_tie _KEYSEQ
         USUBJID PARAMCD AEDECOD CMDECOD
         AESEQ VSSEQ EGSEQ CMSEQ LBSEQ ADT ASTDT;
  run;
...
, ref_lib=ref
  ...
  , sortkeys=USUBJID PARAMCD AVISIT ADT
  , tiebreak=VSSEQ
  , chkvars=STUDYID PARAM VISIT AVISIT ADY AVALU ABLFL TRTA TRTP
  , datevars=TRTSDT TRTEDT
  ...
  %if %sysfunc(exist(&adam_lib..adlb)) and %sysfunc(exist(&ref_lib..ref_adlb)) %then %do;
    ...
    , sortkeys=USUBJID PARAMCD AVISIT ADT
    , tiebreak=LBSEQ
    , chkvars=STUDYID PARAM VISIT AVISIT ADY AVALU ABLFL ONTRTFL ANRIND BASEC BNRIND SHIFT
              ATOXGRH ATOXGRL ATOXGR TRTA TRTP
    , datevars=TRTSDT TRTEDT
